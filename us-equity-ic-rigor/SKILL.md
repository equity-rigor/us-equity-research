...
Phase 4 runs all 17 scripts before sign-off and writes the result to `outputs/<ticker>_verification_gates.json` conforming to `schemas/verification_gates.json`. The `overall_pass` boolean and `blocks_score_above` numeric capture the gate state for the score-band logic above. **v0.1.x memos with schema_version="0.1.0"** are grandfathered to the 14-gate set; G15-G17 are skipped (logged as `skipped: grandfathered_v0_1`).
