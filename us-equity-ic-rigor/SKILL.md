...
This is the iteration that takes a memo from "looks good" to actually defensible. Run the 14 verification gates programmatically (scripts in `scripts/`), then score on the rubric in `references/pm-redteam-rubric-us.md`. The 14-gate verification is non-negotiable: a memo cannot claim score >8.0 with any gate failing; critical math gates G1 (EPS × multiple multiplicativity) and G3 (SOTP monotonicity) cap at 7.0 when they fail. The Phase E success criterion per D20 is score ≥ 8.5 with all 14 gates exit 0 against the structured JSON.
...
## Verification gates (14 gates — run before claiming any score above 8.0)

These are the gates a PM will check, mechanically. If any gate fails, the score caps at 8.0 regardless of how strong the narrative is. Two gates (G1 EPS × multiple multiplicativity and G3 SOTP monotonicity) cap at 7.0 when they fail because they are critical math errors that destroy the memo's arithmetic credibility. Canonical descriptions of all 14 gates live in `schemas/verification_gates.json` under `gate_definitions`; that schema is the contract. Each gate has a Python verification script that exits 0 on pass and non-zero on fail.
...
Phase 4 runs all 14 scripts before sign-off and writes the result to `outputs/<ticker>_verification_gates.json` conforming to `schemas/verification_gates.json`. The `overall_pass` boolean and `blocks_score_above` numeric capture the gate state for the score-band logic above.
...
- Net-new gates G11-G14: non-GAAP reconciliation (G11), SBC-in-FCF disclosure (G12), Barra factor exposure (G13), capacity / ADV / days-to-exit (G14)
