...
**Sector adaptation: banks (no COGS) — per D24**: G2 reconciliation for banks applies to **NIM (net interest margin) × earning-asset weights** at LTM and forward, **NOT** to GM × revenue. Σ(segment_avg_earning_assets × segment_NIM) / Σ(segment_avg_earning_assets) ≈ consolidated NIM within **±5bp** (tighter tolerance than industrials' ±50bp because banks' earning-asset base is precisely measured on the balance sheet, not estimated from segment notes). Efficiency ratio is the complementary T1-T5 dimension (cost-to-serve) but is NOT the G2 trip metric — NIM is. The script `verify_segment_gm.py` treats NIM rows identically to GM rows because the underlying arithmetic is dimensionally the same (a weighted-average reconciliation); the modeler stores NIM in the `value` field with units `%` and weights with the corresponding `segment_avg_earning_assets` field. See `gm-taxonomy-us.md` D24 banks adaptation paragraph for the full T1-T5 NIM/efficiency-ratio mapping. SOTP for banks substitutes PPNR for GP and Pre-Tax Income for OP per `three-method-valuation-us.md` banks SOTP adaptation; G3 monotonicity still applies (NI ≤ Pre-Tax ≤ PPNR ≤ Revenue).
