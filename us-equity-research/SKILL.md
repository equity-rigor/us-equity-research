...
- `references/source-stratification-us.md` — S1-S5 + Pending taxonomy applied to US filings. **Public-source boundary (v0.7.0, G23):** every committed S1-S5 citation must resolve to a publicly accessible source (SEC EDGAR, FRED, Damodaran, public consensus aggregators, company IR, trade press). Entitlement-restricted research (client-only sell-side notes, NDA expert calls, subscription feeds, anything marked no-redistribution / no-AI-input) is **out of framework scope** — usable only as a personal, non-persisted, localized cross-check, never a committed anchor. Public sell-side *signals* (aggregated consensus median/dispersion, public PTs, rating distributions, PT history) are S4 — ingest them as evidence but re-derive the multiple/PT yourself; never adopt another analyst's conclusion. The downstream IC-rigor judgment layer (G21 fair-value reconciliation, G22 revision attribution) builds on this; see `us-equity-ic-rigor/references/judgment-discipline-us.md`.
